...
**絶対禁止（品質ゲートの根幹）**: **`--cov-fail-under` の引き下げ（99% の最終失敗未満へ変更）**。

**CI の最終ゲートは 99%**、ローカル／pre-commit の **99.02% 前後は意図された余裕**（`AGENTS.md`・myrules）。不足分は**テスト追加**のみで埋める。並列段の `--cov-fail-under=0` 等は
