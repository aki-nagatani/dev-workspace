---
name: otayori-navi_pull-request
description: >-
  otayori-navi リポジトリ向け: develop 上のコミット・push、main ← develop の PR 作成・マージ、
  マージ後の GitHub Actions 確認、CursorLog 追記（JST 実時刻）まで。1 チャット 1 回限り、
  pre-commit 必須・--no-verify 禁止。コミットメッセージは commit-message、CI 待機は
  github-actions-check。専用 Cursor Command（旧 otayori-navi/.cursor/commands/）は廃止し
  本 SKILL が正本。otayori-navi PR、デプロイ前マージ、develop push の依頼時に使う。
---

# otayori-navi プルリクエスト（`otayori-navi_pull-request`）

## 概要

**旧 `otayori-navi/.cursor/commands/otayori-navi_pull-request.md` は廃止**し、手順の正本を本 SKILL に置く。`name` は **`otayori-navi_pull-request`**（従来の Command 名と同じ）。

**リポジトリ作業**: `D:/OneDrive/git_work/otayori-navi`（`aki-nagatani/otayori-navi`）。myrules を厳守して作業してください。
...
otayori-navi リポジトリのファイルをコミット＆プッシュしてください。コミットメッセージの作成・確認・修正は **`commit-message` SKILL** を参照してください。
...
5. **プルリクエスト**は **`base` = `main`**、**`head` = `develop`**（同一リポジトリ `aki-nagatani/otayori-navi`）。`head` に `fix/...` 等の別ブランチを指定しない。
...
**絶対禁止（品質ゲートの根幹）**: **`--cov-fail-under` の引き下げ（99 未満へ変更）**。**CI の最終 pytest は 99%**（`deploy.yml` で上書き）、`pytest.ini`／pre-commit の **99.02% 前後は余裕**（`AGENTS.md`・myrules）。不足分は**テスト追加**のみで埋める。「一時的に」や数値が僅差であることは理由にならない。
...
- リポジトリ既定の PR 本文は **`.github/pull_request_template.md`**
  ...
  **`cov-fail-under` 変更が不要な PR では数値行を差分に含めていない**ことを確認する。
  レビューで **`pytest.ini` / pre-commit / workflow** の閾値行に注意する。
