...
**絶対禁止（品質ゲートの根幹）**: **`pytest.ini`**・**`.githooks/pre-commit`・`.github/workflows`** 等における **`--cov-fail-under` の引き下げ（99 未満へ変更）**。**AGENTS.md の正本どおり 99 を維持**し、不足分は**テスト追加**のみで埋める。「一時的に」や数値が僅差であることは理由にならない。
