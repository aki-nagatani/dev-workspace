...
**絶対禁止（品質ゲートの根幹）**: `pyproject.toml`・`.githooks/pre-commit`・`.github/workflows/deploy.yml` 等における **`--cov-fail-under` の引き下げ（99 未満へ変更）**。**AGENTS.md の正本どおり 99 を維持し、不足分はテスト追加のみで埋める。**「一時的に」や数値が僅差であることは理由にならない。
