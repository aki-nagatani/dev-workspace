---
name: FishTrack_pull-request
description: >-
  FishTrack リポジトリ向け: develop 上のコミット・push、main ← develop の PR 作成・マージ、
  マージ後の GitHub Actions 確認、CursorLog 追記（JST 実時刻）まで。1 チャット 1 回限り、
  pre-commit 必須・--no-verify 禁止。コミットメッセージは commit-message、CI 待機は
  github-actions-check。専用 Cursor Command（旧 FishTrack/.cursor/commands/）は廃止し
  本 SKILL が正本。FishTrack PR、FishTrack デプロイ前マージ、develop push の依頼時に使う。
---

# FishTrack プルリクエスト（`FishTrack_pull-request`）

## 概要

**旧 `FishTrack/.cursor/commands/FishTrack_pull-request.md` は廃止**し、手順の正本を本 SKILL に置く。`name` は **`FishTrack_pull-request`**（従来の Command 名と同じ）。

**リポジトリ作業**: `D:/OneDrive/git_work/FishTrack`（`aki-nagatani/FishTrack`）。myrules を厳守して作業してください。
...
FishTrack リポジトリのファイルをコミット＆プッシュしてください。コミットメッセージの作成・確認・修正は **`commit-message` SKILL** を参照してください。
...
5. **プルリクエスト**は **`base` = `main`**、**`head` = `develop`**（同一リポジトリ `aki-nagatani/FishTrack`）。`head` に `fix/...` 等の別ブランチを指定しない。
...
**絶対禁止（品質ゲートの根幹）**: **`--cov-fail-under` の引き下げ（99 未満へ変更）**。**CI（deploy.yml）の最終ゲートは 99% のまま**とし、ローカル／`pyproject.toml`／pre-commit の **99.02% 前後は CI より厳しめの余裕**（`AGENTS.md`・myrules。各リポの実値に合わせる）。不足分はテスト追加のみで埋める。「一時的に」や数値が僅差であることは理由にならない。
...
- リポジトリ既定の PR 本文は **`.github/pull_request_template.md`**
  （カバレッジ閾値の自己チェック含む）。**PR 作成・マージ**の際、本文の
  **チェックリストが実態と一致**していること、および
  **`cov-fail-under` 変更が不要な PR では数値行を差分に含めていない**ことを確認する。
...
マージ完成後、**必ず GitHub Actions の完了を待って結果を確認してください。**
