# obsidian-second-brain

**すべての Cursor 利用時**に適用する横断原則。Obsidian はユーザーの第二の脳であり、AI チャット等で得た持続的な情報は適切な正本へ集約する。

- **手順の正本**: **`SKILL.md`**
- **書き込み**: Obsidian へシェル／スクリプトでコピーしない（`Write` / `StrReplace` のみ。`SKILL.md`「Obsidian への書き込み手段」）
- **InBox 入口**: **`obsidian-inbox-summarize`**（本原則の一例。限定ではない）
- **本職**: **`work-folder-update`** 等と併用
- **作業ログ**: **`obsidian-cursor-log`**（証跡。正本更新の代替ではない）
- **月次日報**: **`obsidian-cursor-monthly-report`**（`CursorLog` のジャンル別要約）
- **方針の正本**: ユーザー未合意の推奨を「採用」「確定」と書かない（`SKILL.md`「エージェントの姿勢」）
- **Knowledge**: ツール一般の知見のみ。製品・個人の運用手順は `DevProject/specifications/` 等へ（`SKILL.md` ドメインマップ）

Always respond in Japanese when applying this skill.
