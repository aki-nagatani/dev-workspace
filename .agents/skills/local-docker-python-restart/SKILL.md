...
6. **`ai-spec-check-preview`**／**`ai-spec-import`** で続けて検証するときは、**Docker 起動時**かつ **プレビュー実行前**に上記 **restart** を済ませる（**`local-docker-python-restart`**・**FishTrack AGENTS**）。**ユーザーへの確認だけで代用しない**。
