...
- **継続的な生成**: **本SKILLをカバレッジ不足の解消目的で起動したとき**は、**コミット（pre-commit）でゲートを通すまで**テストを追加し続ける（必須）。**通常の `src/` 実装セッション**では下記「開発中」と myrules に従い、**セッション終了時の 99.02% 達成は必須としない**。
- **カバレッジ指標**:
  - Python: ブランチ、関数、行、ステートメントすべて99%以上
  - JavaScript: branches, functions, lines, statements すべて99%以上
- **完了条件（カバレッジ解消タスク時）**: pre-commit／CI 相当で閾値以上に達した場合のみ作業を完了とする
- **継続ルール（カバレッジ解消タスク時）**: 閾値未満の場合は、追加のテストコードを生成し続ける

#### サブセット実行時の `--no-cov`（開発中の確認のみ）

- **`pyproject.toml` / `pytest.ini` の既定 `addopts` に `--cov` と `--cov-fail-under` が付くリポジトリ**では、変更に関連する **`pytest tests/foo/test_bar.py`** だけを繰り返し実行すると、**計測対象が一部に限定され** **`cov-fail-under` でプロセス全体が失敗**することがある。\
  **その確認だけなら** **`python -m pytest … --no-cov`** でテストの green/red を見てよい。
- **99.02% ゲートの確認タイミング**: **全体カバレッジ閾値の達成**は **pre-commit（コミット時）** と **CI** で確認する。**開発中・セッション終了時**に毎回フル pytest で **99.02%** を必須としない（dev-workspace **myrules**「テスト規律」）。**恒久対応として閾値を 99 未満に下げることは禁止**（各 **`AGENTS.md`**）。

#### `src/` 変更時（開発中）

**FishTrack**・**MyPokedex**・**おたよりナビ** で **`src/` 配下を変更するとき**は、次を守る。
...
### 5.5 テストファイルの行数と分割（diff-cover 時も必須）

**カバレッジ不足の解消**・**diff-cover 100%** のためにテストを足すときも、**`file-size-policy` SKILL**（**「テストファイル分割（Python・必須）」**）に従う。

- **禁止**: `*_diff_cover.py` 等、**計測ゲート専用**の新規ファイルへの追記・切り出し
- **必須**: 対象 `src` の**振る舞い・関数名・ドメイン**に合わせたファイル名（例: `test_reel_line_capacity_daiwa_row_td.py`）。**理想500行/ファイル**を超えそうなら **先に**分割してから未カバー行用テストを足す
- **編集手段**: 分割・移動は **myrules** どおり **`StrReplace` / `Write` のみ**（PowerShell 行切り詰め・一時スクリプトでの `.py` 書換え禁止）
- **pre-commit**: `check_file_size` 失敗時は **truncate せず**、本節と `file-size-policy` を Read し直して **塊設計からやり直す**
- **自問**: 追加後に `python scripts/check_file_size.py tests/...` を実行し、**1,000行超**なら同一セッション内で分割を完了するか、分割先を決めてからコミットする
- **Task 委譲**: テスト分割をサブエージェントに渡すときは **`file-size-policy` SKILL「委譲時」**に従う。**分割本体は親が `StrReplace`/`Write` で実施**が原則。`shell` サブエージェントに `.py` 分割させない
...
**行数**: 追記先が **1,000行超**または **理想500行超で増え続ける**場合は、**§5.5** と **`file-size-policy`** に従い **振る舞い単位で分割してから**テストを書く（diff-cover 用の別ファイルは作らない）。
