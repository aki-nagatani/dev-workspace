...
Work に関する**ありとあらゆる業務情報**（人・案件・課題・手続き・暗黙知・数値・合意・訂正等）を、**依頼ファイル以外のチャットからも**拾い、**正本ノートへ振り分けて記録**する。\
**チャットで確定した本職情報は、ユーザー指示がなくても同一セッション内で正本へ反映する**（`SKILL.md`「能動的な正本更新」）。\
Work は**第2の脳**—移す価値が少しでもあれば正本を更新する。詳細は **`SKILL.md` § 収集対象・Work フォルダ構成・記録の横断ルール**。
