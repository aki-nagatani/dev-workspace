...
環境変数:
  ...
  TARGET_DB_NAME: 移行先DB名（デフォルト: fishtrack_db）
"""
...
def main() -> int:
    ...
    target_db = os.environ.get("TARGET_DB_NAME", "fishtrack_db")
    ...
    # 2. pg_dump で FishTrack テーブルのみエクスポート（スキーマ+データ）
    table_args = []
    for t in FISHTRACK_TABLES:
    ...
    print(f"Dumping {len(FISHTRACK_TABLES)} tables from {info['dbname']}...")
