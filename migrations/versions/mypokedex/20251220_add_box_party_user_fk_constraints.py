"""add missing FK constraints to box_members and party_members userId columns"""
...
revision = "20251220_add_box_party_user_fk_constraints"
down_revision = "20251220_add_placement_fk_constraints"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    # 1. 孤立レコードを削除（外部キー制約を追加する前に）
    delete_box_orphans_sql = sa.text(
        f"""
        DELETE FROM "{BOX_TABLE}"
        WHERE "userId" NOT IN (SELECT id FROM "{USER_TABLE}")
        """
    )
    deleted_box = conn.execute(delete_box_orphans_sql).rowcount
    
    delete_party_orphans_sql = sa.text(
        f"""
        DELETE FROM "{PARTY_TABLE}"
        WHERE "userId" NOT IN (SELECT id FROM "{USER_TABLE}")
        """
    )
    deleted_party = conn.execute(delete_party_orphans_sql).rowcount
    
    if deleted_box > 0 or deleted_party > 0:
        print(f"警告: 孤立レコードを削除しました - box_members: {deleted_box}件, party_members: {deleted_party}件")
    
    # 2. データ整合性確認（孤立レコードが存在する場合は明示的に失敗させる）
    _assert_no_orphans(conn, BOX_TABLE, "userId", USER_TABLE, "id")
    _assert_no_orphans(conn, PARTY_TABLE, "userId", USER_TABLE, "id")
...
def downgrade() -> None:
    ...
    if _table_exists(inspector, PARTY_TABLE):
        with op.batch_alter_table(PARTY_TABLE, recreate="always") as batch:
            batch.drop_constraint("fk_party_members_user", type_="foreignkey")

    if _table_exists(inspector, BOX_TABLE):
        with op.batch_alter_table(BOX_TABLE, recreate="always") as batch:
            batch.drop_constraint("fk_box_members_user", type_="foreignkey")
